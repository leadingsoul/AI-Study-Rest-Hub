  ...
  <div class="notice-manage-container">
    ...
    <el-dialog v-model="viewDialogVisible" :title="viewNotice.title" width="600px">
      <div class="notice-detail" v-if="viewNotice">
        ...
        <div class="notice-time">创建时间：{{ formatTime(viewNotice.createTime) }}</div>
      </div>
    </el-dialog>
  </div>
</template>
...
const getNoticeList = async () => {
  ...
  try {
    ...
    if (res.code === 200) {
      // 确保每个公告都有createTime字段，如果没有则设置默认值
      noticeList.value = (res.data || []).map(notice => ({
        ...notice,
        createTime: notice.createTime || '未知时间'
      }))
    } else {
    ...
    ElMessage.error('获取公告列表失败，请检查网络连接')
    noticeList.value = []
  } finally {
    loading.value = false
  }
