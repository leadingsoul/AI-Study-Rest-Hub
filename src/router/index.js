...
const routes = [
  ...
  },
  {
    ...
    children: [
      ...
      {
        path: 'ai-guess-manage',
        name: 'AiGuessManage',
        component: () => import('../views/AiGuessManage.vue'),
        meta: { title: '猜词管理' }
      }
    ]
  },
  ...
  // 猜词游戏相关路由
  {
    path: '/ai-guess',
    name: 'AiGuess',
    component: () => import('../views/AIGuess.vue'),
    meta: { title: 'AI猜词游戏' }
  },
  {
    path: '/ai-guess-record',
    name: 'AiGuessRecord',
    component: () => import('../views/AiGuessRecord.vue'),
    meta: { title: '猜词记录' }
  },
...
router.beforeEach((to, from, next) => {
  // 设置页面标题
  if (to.meta.title) {
    document.title = to.meta.title + ' - 智娱研习社'
  }
...
router.afterEach((to) => {
  document.title = to.meta.title + ' - 智娱研习社'  // 更改标题后缀
})
