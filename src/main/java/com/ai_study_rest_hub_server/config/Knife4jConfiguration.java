...
public class Knife4jConfiguration {
    ...
    public OpenAPI customOpenAPI() {
        return new OpenAPI()
                .info(new Info()
                        .title("🎓 智慧学习和休闲系统API文档") // API文档标题，添加emoji图标
                        .description("📚 智慧学习和休闲系统后端接口文档，提供完整的RESTful API服务\n\n" +
                                "✨ 主要功能模块：\n" +
    ...
    //5.猜词游戏管理(猜词界面，猜词分类，猜词题目，猜词记录)4+9+4=17【不实现】
    @Bean
    public GroupedOpenApi guessAPI() {

        return GroupedOpenApi.builder().group("猜词游戏管理").
                pathsToMatch(
                        "/api/guess/category/**",//猜词分类管理
                        "/api/guess/topic/**",//猜词题目管理
                        "/api/guess/record/**",//猜词记录管理
                        "/api/guess/chat"//猜词界面
                ).build();
    }

    //6.系统信息管理（视频和轮播图）十六套接口 【实现轮播图，讲解文件上传】
    @Bean
    ...
    //7.其他信息管理（数据统计，文件查询，debug）四套接口 【实现文件查询】
    @Bean
